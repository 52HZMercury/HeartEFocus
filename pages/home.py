...
class HomePage(QWidget):
    def __init__(self, tabview: TabView, parent=None):
        ...
        self.button2.clicked.connect(lambda: self.tabview.add_tab('智能诊断', ZhinengZhenduanPage(), True, icon=QIcon(binglizhenduan_path)))
        ...
        self.button3.clicked.connect(lambda: self.tabview.add_tab('实时智能监控', ShishiZhiNengJiancePage(), True))
        ...
        self.button4.clicked.connect(lambda: self.tabview.add_tab('大数据分析', DashujuFenxiPage(), True, icon=QIcon(tongjifenxi_path)))
        ...
        title_label = QLabel('欢迎使用乳腺智慧诊断系统')
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet('font-size: 40px; font-weight: bold;')

        description_label = QLabel('Coding Every Day v1.0')
